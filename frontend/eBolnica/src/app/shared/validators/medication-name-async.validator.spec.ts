...
describe('medicationNameAsyncValidator', () => {
  ...
  it('exports user-facing messages for validator error keys', () => {
    expect(MEDICATION_NAME_VALIDATOR_MESSAGES[MEDICATION_NAME_EXISTS_ERROR]).toBe(
      'Medication name already exists'
    );
    expect(MEDICATION_NAME_VALIDATOR_MESSAGES[MEDICATION_NAME_CHECK_UNAVAILABLE_ERROR]).toContain(
      'Unable to verify name availability'
    );
  });
